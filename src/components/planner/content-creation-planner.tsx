...
import ContentPlanList from "./views/content-plan-list";
import { useContentPlans } from "./hooks/use-content-plans";

const ContentCreationPlanner = () => {
  const [viewMode, setViewMode] = useState<"calendar" | "list" | "grid">("calendar");
  const { plans, isLoading, addPlan, updatePlan, deletePlan } = useContentPlans();
    ...
    <div className="space-y-4">
      ...
      <div>
        <TabsContent value="calendar" className="mt-0">
          <MonthlyCalendarView 
            plans={plans.filter(p => p.type === 'content')} 
        ...
        <TabsContent value="grid" className="mt-0">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {plans.filter(p => p.type === 'content').map(plan => (
              <div 
                key={plan.id} 
...
export default ContentCreationPlanner;
